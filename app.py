...
DATA_DIR = ROOT / "data"
VERSAO_DEPLOY = "2026-07-04-sem-referenciais-com-grafo"
...
st.caption(f"Versão publicada: {VERSAO_DEPLOY}")
...
tab_route, tab_legs, tab_model = st.tabs(
    ["Rota", "Trechos", "Modelo"]
)

with tab_route:
    graph_col, route_col = st.columns([2.3, 1])
    with graph_col:
        st.markdown(route_svg(clubs, route), unsafe_allow_html=True)
    with route_col:
        st.subheader("Sequência recomendada")
        sequence_without_origin = [place.name for place in route if place.name != start.name]
        st.write(" → ".join(sequence_without_origin))
...
with tab_model:
    ...
    with model_col:
        ...
        model_frame = pd.DataFrame(model_rows).astype(str)
